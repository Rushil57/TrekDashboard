...
using SCE.Web.DataUtil.Helper;

namespace SCE.Web.DataUtil.Service
{
    public class CloseToBook: BaseRepository
    {
        ...
        public async Task<ArrayList> GetSCEProposalsCount()
        {
            SqlMapper.SetTypeMap(typeof(SCEResult_OneDto), new ColumnAttributeTypeMapper<SCEResult_OneDto>());
            SqlMapper.SetTypeMap(typeof(SCEResult_TwoDto), new ColumnAttributeTypeMapper<SCEResult_TwoDto>());
            ArrayList SCEModelList = new ArrayList();
            using (connection = Get_Connection())
            {
                using (var dataObj = await connection.QueryMultipleAsync("sp_GetSCEProposalsCount", commandType: CommandType.StoredProcedure))
                {
                    SCEModelList.Add(dataObj.Read<SCEResult_OneDto>().ToList());
                    SCEModelList.Add(dataObj.Read<SCEResult_TwoDto>().ToList());
                }
            }
            return SCEModelList;
        }
        ...
        public async Task<int> ApplySCE()
        {
            var status = 0;
            try
            {
                using (connection = Get_Connection())
                {
                    ...
                    status = await connection.ExecuteScalarAsync<int>("sp_ApplySCE", commandType: CommandType.StoredProcedure);
                }
